...
map <- leaflet() %>%
  ...
  addPolygons(data = polygon1,
              ...
              color = "black",
              weight = 2) %>%
  addMarkers(data = towns, 
             lng = ~lon, 
             lat = ~lat, 
             label = ~name,
             icon = whiteDotIcon,
             group = "Towns") %>%
  addLabelOnlyMarkers(data = towns,
                      lng = ~lon, 
                      lat = ~lat, 
                      label = ~name,
                      labelOptions = labelOptions(noHide = T, 
                                                  ...
                                                  style = list("color" = "white", 
                                                    ...
                                                    "line-height" = "1")),
                      group = "Town Names") %>%
  fitBounds(lng1 = min(combined_bbox[1], min(towns$lon)), 
            lat1 = min(combined_bbox[2], min(towns$lat)), 
            lng2 = max(combined_bbox[3], max(towns$lon)), 
            lat2 = max(combined_bbox[4], max(towns$lat))) %>%
  addLegend(
